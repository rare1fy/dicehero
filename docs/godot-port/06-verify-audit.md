...
### 2026-04-27 v2.0 · 设计修正落地

刘叔第 2 轮指令要求修正以下 6 条设计错误，已全部落地到 Part1-Part3 文档中：

| # | 修正内容 | 落地位置 | 状态 |
|---|---|---|---|
| 1 | 篝火：升级遗物 → 回血 / 净化骰子二选一 | Part3 §16.2 | [PASS] |
| 2 | 宝箱：花金币开箱 → 新小玩法（保留基础形式说明）| Part3 §16.4 + Checklist §21.6 | [PASS] |
| 3 | skillSelect 阶段 / 增幅模块全删 | Part1 §1.1 / §1.2 + Part3 §18.7 BGM | [PASS] |
| 4 | 战利品分级：普通/精英/中 Boss/终极 Boss 4 档 | Part3 §16.5 + Checklist §21.8 | [PASS] |
| 5 | 盗贼初始普通骰 3 → 4 | Part1 §3.1 | [PASS] |
| 6 | 骰子升级机制搁置，标注待办 | Part2 §11.7 + Part3 §16.2 | [PASS] |

### 原 5 条 WARN 落地状态

| # | 原 WARN | 当前状态 |
|---|---|---|
| W1 | 敌人 emoji 字段空，直接用像素精灵 | Part2 §10 序言已说明 [RESOLVED] |
| W2 | drawFromBag 是纯函数，Toast/动画由调用方处理 | 原文档已准确描述 [NO-CHANGE] |
| W3 | Boss 小兵从当前章节普通敌人池抽 | 原文档已准确描述 [NO-CHANGE] |
| W4 | DICE_BY_RARITY.uncommon = []，商店不出 uncommon 通用骰 | 原文档已准确描述 [NO-CHANGE] |
| W5 | 初始骰子以 classes.ts 为准，INITIAL_DICE_BAG 是遗留常量 | Part2 §11.8 已标注"职业选择时覆盖" [NO-CHANGE] |

### 新增待办（TASKS）

- `DESIGN-TREASURE-MINIGAME` 宝箱节点小玩法设计（P1）
- `FEATURE-DICE-UPGRADE` 骰子升级体系评估（搁置，未排期）

### 文风改造

所有核心章节（三职业 / 敌人 / 骰子 / 遗物 / 牌型 / 状态 / 挑战 / 节点 / 灵魂晶 / UI / 边界）均已补充"**体验定位**"自然语言序言，避免"只用程序描述"。
