...
import { DiceBagPanel, MiniDice } from './DiceBagPanel';
    ...
    <div className="relative z-[5] player-hud-panel">
      ...
      <div className="px-3 py-1">
        <div className="flex items-center gap-1.5 mb-1">
          <motion.div animate={hpGained ? { scale: [1, 1.1, 1], y: 0 } : playerEffect === 'attack' ? { y: [0, -6, 0] } : { y: 0 }} className="flex items-center gap-1 shrink-0 cursor-pointer leading-none" onClick={() => {}}>
        ...
        <div className={`pixel-hp-bar h-3 relative ${game.statuses.some(s => s.type === 'poison') ? 'animate-poison-pulse' : ''} ${game.statuses.some(s => s.type === 'burn') ? 'animate-burn-edge' : ''}`}>
          <motion.div className={`h-full ${game.armor > 0 ? 'pixel-hp-fill-armor' : getHpBarClass(game.hp, game.maxHp)}`} initial={{ width: '100%' }} animate={{ width: `${(game.hp / game.maxHp) * 100}%` }} />
