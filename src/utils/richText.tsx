...
const KEYWORD_PATTERNS: RegExp[] = [
  ...
  // 触发条件 & 机制关键词
  /出牌时|击杀时|受到伤害时|回合结束时|回合开始时|战斗开始时|战斗结束时|致命伤害时|移动时|卖血[Rr]oll|卖血重投|重[Rr]oll|重投|重掷|被动|首次出牌|溢出伤害|每回合|每次出牌|坍缩|分裂|反噬|自毁|自伤|破甲|穿透|真实伤害|净化|严格递减|完全相同|恰好为|恰好等于|全部手牌|全部≤\d+点|全部同为奇数|全部同为偶数|与上次相反|点数之和|点数和|同一种元素|不构成任何牌型|≥\d+颗骰子|≤\d+|蓄力|过充|狂暴|连击|手牌上限|不同牌型|临时骰子|伤害倍率|可叠加|出牌后重置/g,
  // 骰子名称
...
export const formatDescription = (text: string): React.ReactNode => {
  ...
  // 限制最多3个高亮
  const finalMarkers = deduped.slice(0, 3);
