 ...
 * 规则：
 *   1) 没 icon：直接返回 text
 *   2) text 里有 ": "（遗物/效果名）：渲染成 "名字: <icon>数字"
 *      例：「连击心得: +1」 + 🃏 → 「连击心得: 🃏+1」
 *   3) text 是纯数字（+N / -N / N）：渲染成 "<数字> <icon>" 或 "<icon> 数字"
 *      例：「+5」 + 盾 → 「+5 🛡」（数字在前，让"多少"这个信息最显眼）
 *
...
export function renderFloatText(text: string, icon?: React.ReactNode): React.ReactNode {
  ...
  return (
    <>
      <span>{text}</span>
      <span style={{ display: 'inline-flex', alignItems: 'center', marginLeft: 2 }}>{icon}</span>
    </>
  );
}
